...
const BaseExtractor = {
    ...
    for (const selector of validSelectors) {
      try {
        ...
        if (typeof Logger !== 'undefined') {
          Logger.debug(`[BaseExtractor] Selector error: ${selector}`, e.message);
        }
      }
    }
  ...
  logTiming(section, startTime, metadata = {}) {
    ...
    Logger.debug(`[${section}Extractor] Extraction completed in ${duration}ms`, {
      duration,
      ...metadata
    });
  }
...
const OverlayManager = {
  ...
  setState(newState, data = {}) {
    ...
    const stateHandlers = {
      ...
      [this.states.CACHE_LOADED]: () => {
        ...
        this.showActionButtons({ showRefresh: true, showDetails: false });
      },
  ...
  showActionButtons(options = {}) {
    ...
    if (options.showDetails !== false && detailsBtn) { // Default to showing details
