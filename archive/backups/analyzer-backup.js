 ...
 * Generated on: 2025-07-06T18:11:28.160Z
...
function setStorageData(data) {
  ...
  /* ============================================
   * MODULE: modules/memoryManager.js
   * ============================================ */
/**
...
async function isOwnProfile() {
  ...
  // Original logic for comparing saved profile URL
  return new Promise((resolve) => {
    ...
    chrome.storage.local.get(['linkedinProfile'], (data) => {
      if (data.linkedinProfile) {
        ...
        const savedUrl = data.linkedinProfile.split('?')[0].replace(/\/$/, '');
        resolve(currentUrl === savedUrl);
      } else {
        resolve(false);
      }
    });
...
function safeQuerySelectorAll(element, selector) {
  ...
  /* ============================================
   * MODULE: modules/profileDiscovery.js
   * ============================================ */
/**
...
const ProfileSectionDiscovery = {
  ...
  /* ============================================
   * MODULE: modules/uiComponents.js
   * ============================================ */
/**
 * UI Components for ElevateLI extension
 * Handles badge injection and overlay creation
 */
...
/**
 * Inject the score badge into the LinkedIn profile
 */
async function injectScoreBadge() {
  ...
  const actionsArea = document.querySelector(SELECTORS.PROFILE_ACTIONS);
  if (!actionsArea) {
  ...
  if (document.getElementById('elevateli-score')) {
  ...
  scoreBadge.style.cssText = `
    background: ${isOwn ? COLORS.OWN_PROFILE : COLORS.LINKEDIN_BLUE} !important;
  ...
  scoreBadge.innerHTML = `
    <span id="badge-ai-status" style="display: inline-flex; align-items: center; gap: 4px;">
      <span style="font-size: 16px;">✨</span>
      <span>ElevateLI</span>
    </span>
    <span id="badge-completeness" style="display: none; margin-left: 4px;"></span>
  `;
  
  scoreBadge.onmouseover = () => scoreBadge.style.opacity = '0.9';
  scoreBadge.onmouseout = () => scoreBadge.style.opacity = '1';
  
  // Find the More button or resources button for positioning
  const moreButton = actionsArea.querySelector('button[aria-label*="More"]');
  const resourcesButton = actionsArea.querySelector('a[href*="/resources"]')?.parentElement;
    ...
    if (isOwn) {
      ...
      if (existingOverlay) {
        ...
        if (isCollapsed) {
          ...
          // Update badge color
          scoreBadge.style.background = COLORS.OWN_PROFILE + ' !important';
          // Save state
          ...
          existingOverlay.querySelector('#minimize-analysis').textContent = 'Show Analysis';
          existingOverlay.style.maxHeight = '60px';
          // Update badge color
          scoreBadge.style.background = COLORS.MEDIUM_SCORE + ' !important';
          // Save state
  ...
  // Return badge element for updates
  return scoreBadge;
}

/**
 * Update badge with completeness score
 */
function updateBadgeCompleteness(completeness) {
  const completenessBadge = document.getElementById('badge-completeness');
  if (completenessBadge) {
    completenessBadge.textContent = `${completeness}%`;
    completenessBadge.style.display = 'inline';
  }
}

/**
 * Update badge with AI status
 */
function updateBadgeAIStatus(status) {
  const statusElement = document.getElementById('badge-ai-status');
  if (statusElement) {
    if (status.includes('Analyzing')) {
      statusElement.innerHTML = `<span style="font-size: 16px;">⏳</span> <span>${status}</span>`;
    } else if (status.includes('Quality')) {
      statusElement.innerHTML = `<span style="font-size: 16px;">✨</span> <span>${status}</span>`;
    } else {
      statusElement.innerHTML = `<span style="font-size: 16px;">✨</span> <span>ElevateLI</span>`;
    }
  }
}

/**
 * Inject analysis overlay for own profile
 */
function injectAnalysisOverlay(analysisData) {
  ...
  // Get last analysis if not provided
  if (!analysisData.summary) {
    safeStorageGet(['lastAnalysis'], (data) => {
      if (data.lastAnalysis && data.lastAnalysis.summary) {
        analysisData = data.lastAnalysis;
        createOverlay(analysisData);
      }
    });
  } else {
    createOverlay(analysisData);
 ...
 */
function createOverlay(analysisData) {
  const overlay = createElement('div', {
    id: 'elevateli-overlay',
    style: {
      position: 'fixed',
      bottom: '20px',
      right: '20px',
  ...
  const headerSection = createOverlayHeader(analysisData);
  ...
  // Add scores section
  const scoresSection = createScoresSection(analysisData);
  contentSection.appendChild(scoresSection);
  
  // Add recommendations section
  const recommendationsSection = createRecommendationsSection(analysisData);
  contentSection.appendChild(recommendationsSection);
  
  overlay.appendChild(contentSection);
  
  // Check saved state
  safeStorageGet(['overlayCollapsed'], (data) => {
    if (data.overlayCollapsed) {
      overlay.setAttribute('data-collapsed', 'true');
      contentSection.style.display = 'none';
      overlay.style.maxHeight = '60px';
    }
  });
...
/**
 * Create overlay header with controls
 */
function createOverlayHeader(analysisData) {
  ...
  minimizeBtn.onmouseover = () => minimizeBtn.style.background = '#f3f4f6';
  minimizeBtn.onmouseout = () => minimizeBtn.style.background = 'none';
  
  const minimizeClickHandler = () => {
    const overlay = document.getElementById('elevateli-overlay');
    const content = document.getElementById('analysis-content');
    const isCollapsed = overlay.getAttribute('data-collapsed') === 'true';
    ...
    if (isCollapsed) {
      ...
      minimizeBtn.textContent = 'Show Analysis';
      overlay.style.maxHeight = '60px';
      safeStorageSet({ overlayCollapsed: true });
    }
  };
  
  addManagedEventListener(minimizeBtn, 'click', minimizeClickHandler);
  ...
  addManagedEventListener(closeBtn, 'click', closeClickHandler);
  
  controlsSection.appendChild(minimizeBtn);
  controlsSection.appendChild(closeBtn);
  headerSection.appendChild(controlsSection);
 ...
 */
function createScoresSection(analysisData) {
  const scoresSection = createElement('div', {
    style: {
      display: 'flex',
      gap: '12px',
      marginBottom: '16px'
  ...
  const contentScoreCard = createScoreCard(
    'Content Quality',
    analysisData.contentScore || 0,
    10,
    COLORS.HIGH_SCORE
  );
  
  // Completeness score
  const completenessCard = createScoreCard(
    'Completeness',
    analysisData.completeness || 0,
    100,
    COLORS.MEDIUM_SCORE,
    true
  );
  
  scoresSection.appendChild(contentScoreCard);
  scoresSection.appendChild(completenessCard);
  
  return scoresSection;
}

/**
 * Create individual score card
 */
function createScoreCard(label, score, maxScore, color, isPercentage = false) {
  ...
  }, [isPercentage ? `${score}%` : `${score}/${maxScore}`]);
 ...
 */
function createRecommendationsSection(analysisData) {
  ...
  if (analysisData.summary) {
    const recommendations = parseRecommendations(analysisData.summary);
    const recList = createElement('div', {
      style: {
        display: 'flex',
    ...
    recommendations.slice(0, 3).forEach((rec, idx) => {
      const recItem = createElement('div', {
        style: {
          fontSize: '13px',
          color: '#4b5563',
          paddingLeft: '20px',
      ...
      recItem.appendChild(document.createTextNode(' ' + rec));
  ...
  refreshBtn.onmouseover = () => refreshBtn.style.backgroundColor = '#004182';
  refreshBtn.onmouseout = () => refreshBtn.style.backgroundColor = COLORS.LINKEDIN_BLUE;
  
  const refreshClickHandler = () => {
    safeSendMessage({action: 'calculateScore', url: window.location.href, forceRefresh: true});
  };
  
  addManagedEventListener(refreshBtn, 'click', refreshClickHandler);
  ...
  const detailsBtn = createElement('button', {
    style: {
      ...
      backgroundColor: 'white',
      color: COLORS.LINKEDIN_BLUE,
      border: `1px solid ${COLORS.LINKEDIN_BLUE}`,
      borderRadius: '6px',
      fontSize: '13px',
      fontWeight: '500',
      cursor: 'pointer',
  ...
  detailsBtn.onmouseover = () => {
    detailsBtn.style.backgroundColor = '#f3f4f6';
  };
  ...
  addManagedEventListener(detailsBtn, 'click', detailsClickHandler);
...
/**
 * Parse recommendations from summary text
 */
function parseRecommendations(summary) {
  ...
  /* ============================================
   * MODULE: modules/profileExtractor.js
   * ============================================ */
/**
 * Profile extraction module
 * Handles extracting and processing LinkedIn profile data
 */
...
/**
 * Extract profile and update UI
 */
async function extractAndUpdate(badge, isOwn) {
  // Throttle extraction
  const now = Date.now();
  if (ExtensionState.lastExtraction && (now - ExtensionState.lastExtraction) < TIMINGS.EXTRACTION_THROTTLE) {
  ...
  const aiEnabled = storageData.enableAI !== false && hasApiKey;
  const cacheEnabled = storageData.enableCache !== false;
  const cacheDuration = storageData.cacheDuration || CACHE_SETTINGS.DEFAULT_DURATION_DAYS;
  const lastAnalysis = storageData.lastAnalysis;
  ...
  if (storageData.lastAnalysis && isOwn) {
    ...
    if (cacheEnabled && cacheAge < maxCacheAge && storageData.lastAnalysis.contentScore) {
      ...
      const completeness = calculateQuickCompleteness(discoveryData);
      
      // Update badge with cached scores
      updateBadgeCompleteness(completeness);
      updateBadgeAIStatus(`Quality: ${storageData.lastAnalysis.contentScore}/10`);
      
      // Show overlay with cached data
      if (isOwn) {
        addManagedTimeout(() => {
          injectAnalysisOverlay({
  ...
  try {
    // Update badge to show extracting
    if (badge) {
      updateBadgeAIStatus('Analyzing...');
    }
    ...
    // Store completeness
    ExtensionState.lastCompletenessScore = completeness;
    
    // Update badge with completeness immediately
    updateBadgeCompleteness(completeness);
    
    if (!isOwn) {
      // For non-own profiles, just show completeness
      updateBadgeAIStatus('ElevateLI');
      ExtensionState.isExtracting = false;
      return;
    }
    
    // For own profile, check if AI is enabled
    if (!aiEnabled) {
      console.log('AI not enabled or configured, showing completeness only');
      updateBadgeAIStatus('AI Disabled');
      
      // Check for cached AI analysis even if AI is disabled
      if (lastAnalysis && lastAnalysis.contentScore && lastAnalysis.isPersonal) {
        updateBadgeAIStatus(`Quality: ${lastAnalysis.contentScore}/10`);
        addManagedTimeout(() => {
          injectAnalysisOverlay({
            contentScore: lastAnalysis.contentScore,
            completeness: completeness,
    ...
    console.log('Starting AI extraction for profile');
    ...
    safeSendMessage({
      ...
      if (response && response.contentScore !== undefined) {
        ...
        // Show analysis overlay for own profile
 ...
 */
function calculateQuickCompleteness(discoveryData) {
  let score = 0;
  let maxScore = 0;
  
  // High priority items (10 points each)
  const highPriority = ['photo', 'headline', 'about', 'experience'];
  highPriority.forEach(item => {
    maxScore += 10;
    if (discoveryData[item] && (discoveryData[item] === true || discoveryData[item].exists)) {
      score += 10;
    }
  });
  
  // Medium priority items (5 points each)
  const mediumPriority = ['skills', 'education', 'backgroundBanner', 'location'];
  ...
  // Low priority items (2 points each)
  const lowPriority = ['certifications', 'projects', 'volunteer', 'languages', 'recommendations'];
  lowPriority.forEach(item => {
    maxScore += 2;
    if (discoveryData[item] && discoveryData[item].exists) {
      score += 2;
    }
  });
  
  // Bonus points for quality
  if (discoveryData.headline?.charCount > 50) score += 2;
  if (discoveryData.about?.charCount > 200) score += 3;
  if (discoveryData.experience?.count >= 2) score += 2;
  if (discoveryData.skills?.count >= 5) score += 2;
  
  maxScore += 9; // Total bonus points possible
...
if (typeof chrome !== 'undefined' && chrome.storage) {
  ...
  /* ============================================
   * MODULE: core/cache-manager-fixed.js
   * ============================================ */
/**
  ...
  findSection(selectors, headingText) {
    ...
    if (headingText) {
      ...
      for (const id of possibleIds) {
        ...
        if (anchor) {
          // First try parent section
          const section = anchor.closest('section');
          if (section) {
            Logger.debug(`[BaseExtractor] Found section via anchor: #${id}`);
            return section;
          }
          ...
          while (sibling) {
            // Check if sibling contains a list
            if (sibling.querySelector('ul, .pvs-list')) {
              Logger.debug(`[BaseExtractor] Found section via anchor sibling: #${id}`);
              return sibling;
            }
    ...
    for (const selector of showAllSelectors) {
      try {
        ...
        if (element) {
          ...
          const match = combinedText.match(/Show all (\d+)|\b(\d+) ${sectionName}/i);
          if (match) {
            totalCount = parseInt(match[1] || match[2]);
          }
    ...
    if (totalCount === 0) {
      ...
      for (const el of textElements) {
        ...
        const patterns = [
          ...
          new RegExp(`Show all\\s+(\\d+)`, 'i'),
          new RegExp(`View all\\s+(\\d+)`, 'i')
        ];
...
const SkillsExtractor = {
  ...
  async scan() {
    ...
    if (exists) {
      ...
      Logger.debug('[SkillsExtractor] Section found, checking for skill items');
      Logger.debug('[SkillsExtractor] Section HTML preview:', section.innerHTML.substring(0, 500));
      
      // Count visible skill items
      for (const selector of this.skillItemSelectors) {
        const items = section.querySelectorAll(selector);
        Logger.debug(`[SkillsExtractor] Trying selector "${selector}" - found ${items.length} items`);
        if (items.length > 0) {
      ...
      if (visibleCount === 0) {
        ...
        Logger.debug(`[SkillsExtractor] Found ${allLists.length} ul elements in section`);
        
        allLists.forEach((list, index) => {
          const listItems = list.querySelectorAll('li');
          Logger.debug(`[SkillsExtractor] List ${index} has ${listItems.length} items`);
          if (listItems.length > 0 && visibleCount === 0) {
            // Check if this looks like a skills list
            const firstItem = listItems[0];
            const hasSkillLink = firstItem.querySelector('a[data-field="skill_card_skill_topic"]');
            // Check for general skill patterns - look for short text items that don't look like navigation
            const itemText = firstItem.textContent.trim();
            const looksLikeSkill = itemText.length > 2 && itemText.length < 100 && 
                                  !itemText.includes('Show all') && 
                                  !itemText.includes('see more') &&
                                  !itemText.includes('Load more');
            if (hasSkillLink || looksLikeSkill) {
...
const FeaturedExtractor = {
  ...
  /* ============================================
   * MODULE: scoring/completeness-scorer-fixed.js
   * ============================================ */
/**
