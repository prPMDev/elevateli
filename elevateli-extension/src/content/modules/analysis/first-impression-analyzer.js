...
const FirstImpressionAnalyzer = {
  ...
  buildFirstImpressionData(extractedData) {
    ...
    Logger.info('[FirstImpressionAnalyzer] Built first impression data:', {
      hasHeadline: !!data.headline,
      headlineLength: data.headline.length,
      hasPhoto: data.photo.exists,
      hasCustomBanner: data.banner.isCustomBanner,
      openToWork: data.metadata.openToWork
    });
  ...
  calculateUnifiedScore(elementScores) {
    ...
    Logger.info('[FirstImpressionAnalyzer] Calculated unified score:', {
      elementScores,
      scoreDetails,
      totalWeight,
      weightedSum,
